...
export function NavV3() {
  ...
  return (
    <header className="fixed inset-x-0 top-0 z-50 flex justify-center px-4 pt-4">
      ...
      >
        <a href="#top" className="flex items-center gap-2.5" aria-label="RankForge home">
          <NavLogo />
      ...
      <AnimatePresence>
        {open && (
          <motion.div
            ...
            exit={{ opacity: 0, y: -10 }}
            className="absolute left-4 right-4 top-20 z-50 rounded-2xl glass-strong p-3 lg:hidden"
          >
            {LINKS.map((l) => (
              <a
                key={l.href}
...
function NavLogo() {
  return (
    <span className="relative grid h-8 w-8 place-items-center rounded-[10px] bg-gradient-to-br from-iris-500 to-iris-700 shadow-[0_4px_16px_-4px_rgba(124,58,237,0.8),inset_0_1px_0_rgba(255,255,255,0.3)]">
